...
Based on the provided code diff, create a concise but descriptive stash message following the instructions above.`,d=`You are an advanced AI programming assistant and are tasked with summarizing code changes into a concise and meaningful title and description. You will be provided with a code diff and optional additional context. Your goal is to analyze the changes and create a clear, informative title and description that accurately represents the modifications made to the code.
...
Based on the provided code diff and message, create a focused technical summary following the format above.`,u=`You are an expert at creating changelogs in the "Keep a Changelog" format (https://keepachangelog.com). Your task is to create a set of clear, informative changelog entries.
...
Based on the provided commit messages and associated issues, create a set of markdown changelog entries following the instructions above. Do not include any explanatory text or metadata`;function p(e,t){switch(e){case"generate-commitMessage":return{name:"Generate Commit Message",template:o,variables:["diff","context","instructions"]};case"generate-stashMessage":return{name:"Generate Stash Message",template:s,variables:["diff","instructions"]};case"generate-changelog":return{name:"Generate Changelog (Preview)",template:u,variables:["data","instructions"]};case"generate-create-cloudPatch":return{name:"Create Cloud Patch Details",template:d,variables:["diff","context","instructions"]};case"generate-create-codeSuggestion":return{name:"Create Code Suggestion Details",template:c,variables:["diff","context","instructions"]};case"generate-create-pullRequest":return{name:"Generate Pull Request Details (Preview)",template:r,variables:["diff","data","context","instructions"]};case"explain-changes":return{name:"Explain Changes",template:l,variables:["diff","message","instructions"]};default:return}}let m=["diff"];function h(e,t,n,o){let r=Object.entries(n).filter(([e])=>t.variables.includes(e)),s=t.template.length+(0,i.cz)(r,([,e])=>e.length),d=n,c=!1;for(let e of(s>o&&(c=!0,r=r.map(([e,n])=>{if(!m.includes(e))return[e,n];let i=o-(s-n.length);if(i>n.length)throw Error(`Unable to truncate context to fit within the ${t.name} limits`);return[e,n.substring(0,i)]})),t.variables))r.some(([t])=>t===e)||r.push([e,""]);return d=Object.fromEntries(r),{content:(0,a.GW)(t.template,d),truncated:c}}}};